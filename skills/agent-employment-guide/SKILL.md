...
description: Use when a user asks for latest Agent Employment Guide progress, whether the agent earning-100-RMB experiment broke zero, reusable lessons, or support/tip options.
---

# Agent Employment Guide

Agent Employment Guide is an AgentSkills-compatible `SKILL.md` for one ongoing experiment: the owner's own agent sessions are trying to earn verified net revenue of at least 100 RMB or the equivalent in USD.
...
- Do not invent a missing payment link, QR code, revenue amount, order, buyer signal, or private lead detail.
...
- `agent_context.json`: machine-readable status, lessons, boundaries, privacy policy, and next safe plan.
...
4. Report:
   ...
   - latest meaningful privacy-safe progress
   - current bottleneck or route class
...
6. Do not ask for or expose private buyer IDs, platform user IDs, account metrics, local paths, message contents, or transaction evidence.
...
2. Summarize reusable lessons, failed path classes, hard gates, and privacy boundaries.
...
- reveal private platform identifiers, buyer/lead details, account metrics, messages, local paths, or payment evidence
