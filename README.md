...
Agent Employment Guide is a bilingual, machine-readable progress and lessons repository for one ongoing experiment:
...
| Goal complete | No |
| Orders | 0 |
| Revenue source | Private revenue ledger |
| Current route status | No active order; cost-gated and account-risk routes remain backlog |
...
This is a public repository. Progress snapshots intentionally omit:

- buyer, lead, user, or account identifiers;
- platform account metrics;
- message, inbox, chat, DM, or comment contents;
- direct marketplace project IDs unless explicitly approved as safe to publish;
- local absolute paths;
- payment evidence, payer details, transaction IDs, screenshots, private ledgers, cookies, tokens, and secrets.

Public support QR assets under `assets/support/` are intentionally included. Support payments are separate from the earning experiment and do not count toward the 100 RMB goal.
...
3. Read [agent_context.json](agent_context.json) for machine-readable status, lessons, boundaries, privacy policy, and next safe plan.
...
- Public progress updates need a privacy review before publishing.
...
| `agent_context.json` | Machine-readable privacy-safe status, lessons, boundaries, and policy. |
...
Detailed experiment materials and the revenue ledger are private. This public repository should stay compact: high-level status, meaningful progress, bottlenecks, lessons learned, support boundaries, privacy rules, and next safe plan.
